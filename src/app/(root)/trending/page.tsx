...
const page = async ({
  ...
  const { results: trending, total_pages } = await GetAllTrending(
    searchParams.page
  );
  const { results: trendRecomm } = await GetAllTrending("3");
  return (
    <div className="flex flex-col lg:flex-row px-5 gap-5 pt-16">
      ...
      <div className="basis-1/4 flex w-full flex-col justify-items-stretch mt-10">
        ...
        <div className="relative flex flex-col gap-3 w-full lg:w-80">
           {trendRecomm.filter((e: any,i: any) => i < 10).map((item: any, index: number) => (
            <LongCards
            key={item.title}
            title={item.title || item.name}
            img={item.poster_path}
