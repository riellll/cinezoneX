...
import { GetMovieRecommendations, GetPopularMovie } from "@/lib/fetchData";
import { IoIosPlay } from "react-icons/io";
import { Metadata } from 'next'

export const metadata: Metadata = {
  title: 'Popular Movies - The Cinizone Collection',
...
const page = async ({
  ...
  const { results: popularMovie, total_pages } = await GetPopularMovie(
    searchParams.page
  );
  const { results: movieRecomm } = await GetMovieRecommendations(901362);
  return (
    <div className="flex flex-col lg:flex-row px-5 gap-5 pt-16">
      ...
      <div className="basis-1/4 flex w-full flex-col justify-items-stretch mt-10">
        ...
        <div className="relative flex flex-col gap-3 w-full lg:w-80">
          {movieRecomm.filter((e: any,i: any) => i < 10).map((item: any, index: number) => (
            <LongCards
            key={item.title}
            title={item.title || item.name}
            img={item.poster_path}
