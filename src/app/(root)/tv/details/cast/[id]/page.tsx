...
import { GetTvCredits, GetTvDetails } from '@/lib/fechTvDetails';
...
export async function generateMetadata(
  ...
  return {
    title: `${tvDetails.name}(${tvDetails.first_air_date.split('-')[0]}) - Cast & Crew - The Cinizone Collection`,
...
const page = async ({ params }: { params: { id: string } }) => {
    ...
    const {cast,crew} = await GetTvCredits(params.id);
  ...
  return (
    ...
    <div className="flex flex-col items-start gap-1 pt-1">
     <h2 className="text-4xl font-extrabold dark:text-white">{movieDetails.original_name}<span className="font-normal">{`(${movieDetails.first_air_date.split('-')[0]})`}</span></h2>
