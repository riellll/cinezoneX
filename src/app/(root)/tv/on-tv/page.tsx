      ...
      <div className="flex w-full flex-col mt-10">
        <div className="flex flex-col sm:flex-row justify-start gap-5 items-center">
            <div className="flex justify-center items-center">
              <p className="bg-green-700 py-1 md:py-1.5 px-1 rounded mr-1"><IoIosPlay /></p>
            <p className="text-xl md:text-3xl font-bold text-gray-200">CURRENTLY AIRING TV SHOWS</p>
            </div>
        </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-6 sm:gap-5 items-center justify-between min-[320px]:grid-cols-2 min-[320px]:gap-3 min-[320px]:mb-10 min-[320px]:mt-10">
               ...
               <div className="flex justify-center items-center text-center">
                <Paginations currentPage={searchParams.page}/>
                </div> 
      </div>
  ...
  <div className="hidden pr-10 md:block">
      ...
      <div className="flex justify-start items-center mb-7">
        <div className="flex justify-center items-center mr-5">
          <p className="bg-green-700 py-1 md:py-1.5 px-1 rounded mr-1"><IoIosPlay /></p>
        <p className="text-xl md:text-3xl font-bold text-gray-200">SUGGESTIONS</p>
        </div>
    </div>
    <div className="relative flex flex-col gap-3 w-full lg:w-80">
      {list.map((item: any, index: number) => (
    <LongCards key={item.title} title={item.title} img={item.img} price={item.price} index={null} />
          ))}
    </div>
    </div>
